...
# version <1.0.1 needs to use the old import path
new_import=true
if [ "$(echo $VERSION | cut -c1)" -eq 0 ] 2>/dev/null || [ "$VERSION" = "1.0.0" ]; then 
    IMPORT="github.com/mholt/caddy" && new_import=false
fi

# vsersion 2.0.0 and above needs to use a new build process
caddy_2=false
if [ "$(echo $VERSION | cut -c1)" -eq 2 ] 2>/dev/null; then 
    caddy_2=true
fi
...
caddy2() {
	# xcaddy
	stage "installing xcaddy"
	GOOS=linux GOARCH=amd64 go get -v github.com/caddyserver/xcaddy/cmd/xcaddy
	end_stage
